  ...
  const fetchPost = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setIsCustomSlug(true); // 既存の投稿はカスタムslugとして扱う
      } else {
        setError('投稿が見つかりません');
      }
    } catch (error) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <div className="mt-2 text-sm text-gray-500">
            <p className="mb-1">プレビュー: <code className="bg-gray-100 px-1 rounded">/blog/{slug ? replaceVariables(slug) : 'post-' + Date.now()}</code></p>
